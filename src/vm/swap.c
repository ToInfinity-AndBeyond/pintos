...
void swap_in(size_t used_index, void *paddr)
{
    ...
    if(bitmap_test(swap_bitmap, used_index))
    {
        ...
        {
            block_read(swap_block, BLOCK_PER_PAGE*used_index+i, BLOCK_SECTOR_SIZE*i+paddr);
        }
...
size_t swap_out(void *paddr)
{
    ...
    if(BITMAP_ERROR != swap_index)
    {
        for(int i=0; i<BLOCK_PER_PAGE; i++)
        {
            block_write(swap_block, BLOCK_PER_PAGE*swap_index+i, BLOCK_SECTOR_SIZE*i+paddr);
        }
